...
export async function writePMMLFilesForModel(modelName: string) {
  ...
  for (
    ...
    const { name, folderPath } = algorithmNamesAndFolderPaths[algorithmIndex];
    ...
    const webSpecificationsCsvString = modelConfig.useMsw
      ...
      : fs.readFileSync(`${modelFolderPath}/web-specifications.csv`, "utf8");
    ...
    const pmml: IPmml = Object.assign(
      {
        ...
        DataDictionary: modelConfig.useMsw
          ? await constructDataDictionaryNodeForMSW(
              ...
              fs.readFileSync(`${folderPath}/local-transformations.xml`, "utf8")
            )
        ...
        ...makeCustomPmmlNode(generalRegressionModel, referenceCsvString)
      },
